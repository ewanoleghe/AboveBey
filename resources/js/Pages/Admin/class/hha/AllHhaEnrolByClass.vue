<template>
    ...
    <div class="flex flex-wrap mx-2 justify-center">
            ...
            <div class="flex-none w-full md:w-1/12 mb-2 md:mb-4 p-4 rounded-lg min-w-[220px]">
                <h1 class="text-2xl font-bold">{{ getDate(dataEnrolHha.data[0]?.course_start) }} <br>{{ "HHA Class" }}</h1>
            </div>
            <!-- column 2 -->
                ...
                <div class="flex flex-col md:flex-row justify-center space-x-0 md:space-x-4 h-full">
                    <div class="w-full md:w-6/6 bg-gray-200 flex flex-col justify-between mb-4 md:mb-0">
                        <div class="p-4 flex-grow">
                            <h2 class="text-xl font-bold">All Registration for {{ getDate(dataEnrolHha.data[0]?.course_start) }}</h2> 
                            ...
                            <div class="overflow-x-auto p-2">
                             ...
                             <table class="min-w-full border-collapse">
                                    ...
                                    <tbody>
                                      <tr v-for="dataEnrolHha in dataEnrolHha.data" :key="dataEnrolHha.id">
                                        <td class="text-black text-center border border-black">
                                                <span 
                                                    class="cursor-pointer inline-flex items-center" 
                                                    @click="toggleReadStatus(dataEnrolHha)"
                                                >
                                                    <img
                                                    :src="dataEnrolHha.read_unr === 'read' ? env_read : env_unread"
                                                    alt="Envelope"
                                                    class="w-6 h-6"
                                                    />
                                                </span>
                                            ...
                                            <td class="text-black text-center border border-black">
                                                <span class="text-x text-black-900 font-bold">{{ dataEnrolHha.first_name }}</span><br>
                                            </td>
                                            <td class="text-black text-center border border-black">
                                                <span class="text-x text-black-900 font-bold">{{ dataEnrolHha.last_name  }}</span><br>
                                            </td>
                                            <td class="text-black text-center border border-black">
                                                <span class="text-x font-bold text-black-900">{{ getDate(dataEnrolHha.course_start)  }}</span>
                                            </td>
                                            <td class="text-black justify-center border border-black pl-2">
                                                <span class="text-x text-black-900 font-bold">{{ dataEnrolHha.address  }}</span><br>
                                                <span class="text-x text-black-900">{{ dataEnrolHha.city  }}, {{ dataEnrolHha.county  }}, {{ dataEnrolHha.postcode  }}</span>
                                            </td>
                                            <td class="text-black text-center border border-black">
                                                <span class="text-xs text-black-900 font-bold">{{ dataEnrolHha.phone  }}</span><br>
                                            </td>
                                            <td class="text-black text-center border border-black">
                                                <span class="text-x text-black-900 font-bold">${{ dataEnrolHha.application_fee  }}</span><br>
                                                <span class="text-xs text-black-900 font-bold" 
                                                    :class="dataEnrolHha.payment_status === 'paid' ? 'bg-lime-300' : 'bg-red-400'">
                                                    {{ dataEnrolHha.payment_status }}
                                                </span>
                                            </td>
                                            <td class="text-black text-center border border-black">
                                                <span class="text-x text-black-900">{{ dataEnrolHha.email  }}</span><br>
                                            </td>
                                            <td 
                                                :class="[dataEnrolHha.payment_status === 'paid' ? 'bg-red-400' : 'bg-lime-300', 
                                                        'text-black', 'text-center', 'font-bold', 'text-xs', 'border', 'p-2', 'border-black']">
                                                <button 
                                                    @click="updatePaymentStatus(dataEnrolHha.id)"
                                                    class="text-black">
                                                    {{ dataEnrolHha.payment_status === 'paid' ? 'Set Pending' : 'Set Paid' }}
                                                </button>
                                            </td>
                                            <td class="bg-orange-200 text-black text-center font-bold border p-2 border-black">
                                                <Link :href="route('admin.view_hha_application')" 
                                                    method="post"
                                                    :data="{ id: dataEnrolHha.id }"
                                                    as="button"
                                                    type="button"
                                                    class="text-black">
                                                    View User &#129195;
                                            ...
                                            <td class="bg-gray-700 text-red-500 text-center font-bold border p-2 border-black">
                                                <Link :href="route('admin.delete_hha_application')" 
                                                    method="post"
                                                    :data="{ id: dataEnrolHha.id }"
                                                    as="button"
                                                    type="button"
                                ...
                                <div>
                                    ...
                                    <PaginationLinks :paginator="dataEnrolHha" />
                                </div>
                            </div>
...
const updatePaymentStatus = async (id) => {
    try {
        ...
        await router.post('/admin/update_hha_payment_status', { id }, { preserveScroll: true });
    } catch (error) {
...
// Toggle Read Status
const toggleReadStatus = async (dataEnrolHha) => {
  try {
    const response = await router.post('/admin/update_hha_read_status', 
      { id: dataEnrolHha.id }, 
      { preserveScroll: true }
    );
    ...
    dataEnrolHha.read_unr = dataEnrolHha.read_unr === 'read' ? 'unread' : 'read';
  } catch (error) {
