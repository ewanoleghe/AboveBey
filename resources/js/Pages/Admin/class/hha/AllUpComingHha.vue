<template>
    ...
    <div class="flex flex-wrap mx-2 justify-center">
            ...
            <div class="flex-none w-full md:w-1/12 mb-2 md:mb-4 p-4 rounded-lg min-w-[220px]">
                <h1 class="text-2xl font-bold">{{ "All Valid" }} <br>{{ "HHA Schedule" }}</h1>
            </div>
            <!-- column 2 -->
                ...
                <div class="flex flex-col md:flex-row justify-center space-x-0 md:space-x-4 h-full">
                    <div class="w-full md:w-6/6 bg-gray-200 flex flex-col justify-between mb-4 md:mb-0">
                        <div class="p-4 flex-grow">
                            <h2 class="text-xl font-bold">All HHA Schedule</h2> 
                            ...
                            <div class="overflow-x-auto p-2">
                             ...
                             <table class="min-w-full border-collapse">
                                    ...
                                    <tbody>
                                      <tr v-for="hhaClass in hhaUpData.data" :key="hhaClass.id">
                                        <td class="bg-gray-600 text-white text-center border border-black">
                                                <span class="text-xl text-white-500 mt-0">{{ getDate(hhaClass.course_start) }}</span><br>
                                            </td>
                                            <td class="bg-gray-700 text-white text-center border border-black">
                                                <span class="text-xl text-white-500 mt-0">{{ getDate(hhaClass.course_end) }}</span><br>
                                            </td>
                                            <td class="text-black text-center border border-black">
                                                <span class="text-xs font-bold text-black-900">{{ formatTime(hhaClass.time_start) }}</span><br>
                                                -<br>
                                                <span class="text-xs font-bold text-black-900">{{ formatTime(hhaClass.time_end) }}</span>
                                            </td>
                                            <td class="text-black text-center border border-black">
                                                <span class="text-xl text-black-900 font-bold">{{ hhaClass.training_code }}</span><br>
                                                <span class="text-xs text-black-900">({{ hhaClass.location }})</span>
                                            </td>
                                            <td class="text-black text-center border border-black">
                                                <span class="text-x text-black-900 font-bold">{{ hhaClass.class_type }}</span><br>
                                            </td>
                                            <td class="text-black text-center border border-black">
                                                <span class="text-xl text-black-900 font-bold">${{ hhaClass.application_fee }}</span><br>
                                            </td>
                                            <td class="text-black text-center border border-black">
                                                <span class="text-x text-black-900 font-bold">{{ hhaClass.enrollments_count > 0 ? hhaClass.enrollments_count : '--' }}</span><br>
                                                <span class="text-xs text-blue-900 font-bold" v-if="hhaClass.enrollments_count > 0">
                                                    <Link :href="route('admin.view_enrol_by_hha_class')" 
                                                        method="get"
                                                        :data="{ id: hhaClass.id }"
                                                        as="button"
                                                        type="button"
                                            ...
                                            <td :class="isCourseValid(hhaClass.course_start) ? 'bg-green-500 text-white' : 'bg-yellow-300 text-black'" 
                                            class="text-center border border-black">
                                                <span class="text-x text-black-900 font-bold">
                                                    {{ isCourseValid(hhaClass.course_start) ? 'Valid' : 'Expired' }}
                                                </span><br>
                                                <span>{{ getDate(hhaClass.course_start) }}</span> <!-- If you still want to show the date -->
                                            </td>
                                            <td class="bg-red-500 text-black-500 text-center font-bold border p-2 border-black">
                                                <Link :href="route('admin.delete_hha_class')" 
                                                    method="post"
                                                    :data="{ id: hhaClass.id }"
                                                    as="button"
                                                    type="button"
                                ...
                                <div>
                                    ...
                                    <PaginationLinks :paginator="hhaUpData" />
                                </div>
                            </div>
