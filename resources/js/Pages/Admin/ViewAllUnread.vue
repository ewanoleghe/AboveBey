<template>
    ...
    <div class="flex flex-wrap mx-2 justify-center">
            ...
            <div class="flex-none w-full md:w-1/12 mb-2 md:mb-4 p-4 rounded-lg min-w-[220px]">
                <h1 class="text-2xl font-bold">{{ "Unread CNA" }} <br>{{ "Registeration" }}</h1>
            </div>
            <!-- column 2 -->
                ...
                <div class="flex flex-col md:flex-row justify-center space-x-0 md:space-x-4 h-full">
                    <div class="w-full md:w-6/6 bg-gray-200 flex flex-col justify-between mb-4 md:mb-0">
                        <div class="p-4 flex-grow">
                            <h2 class="text-xl font-bold">Unread CNA Registration</h2> 
                            ...
                            <div class="overflow-x-auto p-2">
                             ...
                             <table class="min-w-full border-collapse">
                                    ...
                                    <tbody>
                                      <tr v-for="cnaReg in unReadRegistration.data" :key="cnaReg.id">
                                        <td class="text-black text-center border border-black">
                                                <span 
                                                    class="cursor-pointer inline-flex items-center" 
                                                    @click="toggleReadStatus(cnaReg)"
                                                >
                                                    <img
                                                    :src="cnaReg.read_unr === 'read' ? env_read : env_unread"
                                                    alt="Envelope"
                                                    class="w-6 h-6"
                                                    />
                                                </span>
                                            ...
                                            <td class="text-black text-center border border-black">
                                                <span class="text-x text-black-900 font-bold">{{ cnaReg.first_name }}</span><br>
                                            </td>
                                            <td class="text-black text-center border border-black">
                                                <span class="text-x text-black-900 font-bold">{{ cnaReg.last_name  }}</span><br>
                                            </td>
                                            <td class="text-black text-center border border-black">
                                                <span class="text-x font-bold text-black-900">{{ getDate(cnaReg.course_start)  }}</span>
                                            </td>
                                            <td class="text-black justify-center border border-black pl-2">
                                                <span class="text-x text-black-900 font-bold">{{ cnaReg.address  }}</span><br>
                                                <span class="text-x text-black-900">{{ cnaReg.city  }}, {{ cnaReg.county  }}, {{ cnaReg.postcode  }}</span>
                                            </td>
                                            <td class="text-black text-center border border-black">
                                                <span class="text-xs text-black-900 font-bold">{{ cnaReg.phone  }}</span><br>
                                            </td>
                                            <td class="text-black text-center border border-black">
                                                <span class="text-x text-black-900 font-bold">${{ cnaReg.application_fee  }}</span><br>
                                                <span class="text-xs text-black-900 font-bold" 
                                                    :class="cnaReg.payment_status === 'paid' ? 'bg-lime-300' : 'bg-red-400'">
                                                    {{ cnaReg.payment_status }}
                                                </span>
                                            </td>
                                            <td class="text-black text-center border border-black">
                                                <span class="text-x text-black-900">{{ cnaReg.email  }}</span><br>
                                            </td>
                                            <td 
                                                :class="[cnaReg.payment_status === 'paid' ? 'bg-red-400' : 'bg-lime-300', 
                                                        'text-black', 'text-center', 'font-bold', 'text-xs', 'border', 'p-2', 'border-black']">
                                                <button 
                                                    @click="updatePaymentStatus(cnaReg.id)"
                                                    class="text-black">
                                                    {{ cnaReg.payment_status === 'paid' ? 'Set Pending' : 'Set Paid' }}
                                                </button>
                                            </td>
...
// Toggle Read Status
const toggleReadStatus = async (cnaReg) => {
  try {
    ...
    cnaReg.read_unr = cnaReg.read_unr === 'read' ? 'unread' : 'read';
  } catch (error) {
